...
app with fastlane and Jenkins. I have come to regret our decision to use
fastlane in that process and have since migrated us to a new system which I will
detail below.

[fast]: {% post_url 2018-08-13-osx-jenkins %}
...
workspaces clean, that's 65 dependencies to install on the build machine for
_every push_.
...
ended up doing a lot of build debugging myself. Here's the problme: Xcode is a
...
This is petty, so I'll keep it brief.

What is it with the emoji? I don't understand the obsession. fastlane's logs,
github, and documentation are littered with them. Do you know why I'm reading
your logs, github, or documentation 9 times out of 10?

**BECAUSE MY APP ISN'T BUILDING AND I'M LOSING MONEY.**

Seeing a cutesy rocket ship doesn't really offset that. I'll go ahead and throw
aliases like `scan` and `gym` into the same bucket. I'm all for having short,
memorable software names but these are essentially method calls in fastlane's
API and they both have sensible real names: `run_tests` and `build_ios_app`. Why
make your tool more confusing just so it can be cute?

## Replacing fastlane

So we ditched fastlane. What now? I replaced it with four scripts:
...
increase in code size is utterly dwarfed by the 65 dependencies we were able to

...
it in terms of light intensity&mdash;crammed all of the details into what were
...
The other, more tricky exception is font rendering. Fonts employ a number
...
removing artistic flourishes that would hamper readability.

In one sense this does have a real world ananlogue. A small font is kind of like
...
procedural_canvas('#invertrgb', function(i, j, h, w) {
    if (i < h / 2) {
        return [j, j, j, 255];
    }
    return [255 - j, 255 - j, 255 - j, 255];
});

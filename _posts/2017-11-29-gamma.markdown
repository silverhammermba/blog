...
What complicates the issue somewhat is that there are many
...
To understand why this is incorrect, think about this: when
...
manipulation where color distortion is _intended_ in order to achieve a certain
...
with digital images. So the steps should be:
...
doesn't account for green being perceived as bright. The incorrect BT.709
...
and with red/yellow on the right.
...
brights meaning we can store the maximum number of _discernable differences in
...
red/blue example). I must emphasize that this isn't a case of sRGB being more or
less **correct**; mixing colors in the sRGB space is **always incorrect**. This
is about sRGB being more or less **intuitive** when it comes to the naive
assumption that perceived brightness and light intensity are the same.
...
specific instructions for it elsewhere.
...
`-colorspace sRGB` gamma encodes it correctly:
    ...
    convert -size 200x200 gradient:red-blue -set colorspace RGB -colorspace sRGB out
...
correction (they call this `rec709luma`). Annoyingly, that is the formula used
...
will actually hurt us here. Let's do an example. We take a black/white
...
intensity and then convert back to sRGB? Look at the result below:

<canvas id="invert" width="256" height="80"></canvas>

It looks like a light intensity-based gradient! Why is this bad? Well remember
that the dark colors are where the human eye best perceives details. We've taken
a gradient that had a perceptually smooth transition and&mdash;by inverting
it&mdash;crammed all of the details into what were the very brightest colors
before. And all of the darker shades in the lower half which used to be easily
distinguishable are now nearly-identical white!

There is no real-world way to invert a color, so using a light intensity-based
calculation makes no sense. The only way to judge the quality of an inversion is
by how we perceive it! So in that sense, naively inverting the sRGB values is
right on!
...
A very extensive article with a focus on image resizing. Good examples of real
world images with noticable distortion due to incorrect color space handling.

[Computer Color is Broken](https://www.youtube.com/watch?v=LKnqECcg6Gw)

A nicely animated explanation with a focus on image bluring.

[What every coder should know about gamma](http://blog.johnnovak.net/2016/09/21/what-every-coder-should-know-about-gamma/)

Really thorough article with even more examples than I have here.

[Resizing with Colorspace Correction](http://www.imagemagick.org/Usage/resize/#resize_colorspace)

The ImageMagick manual's section on correctly resizing images, with some notes
about other linear color spaces.
...
function update_interactive() {
    ...
    procedural_canvas('#interactive', function(i, j, h, w) {
        var p = j / w;
...
procedural_canvas('#compare', function(i, j, h, w) {
    var edgesize = 80;
	if (j <= edgesize) {
		var color = (i + j) % 2 ? 0 : 255;
		return [color, color, color, 255];
	...
	if (j > w / 2 && j < w - edgesize) return [i, i, i, 255];
	return [127, 127, 127, 255];
});
...
procedural_canvas('#compare-rgbry', function(i, j, h, w) {
    var edgesize = 80;
	if (j <= edgesize) {
		var color = (i + j) % 2 ? 0 : 255;
	...
	if (j > w / 2 && j < w - edgesize) return [255, i, 0, 255];
	return [255, 127, 0, 255];
});
procedural_canvas('#compare-rgbrb', function(i, j, h, w) {
    var edgesize = 80;
	if (j <= edgesize) {
		var color = (i + j) % 2 ? 0 : 255;
		return [255 - color, 0, color, 255];
	}
	if (j > edgesize && j <= w / 2) {
		var c1 = to8bit(linear2gamma(i / 255));
		var c2 = to8bit(linear2gamma(1 - i / 255));
		return [c2, 0, c1, 255];
	}
	if (j > w / 2 && j < w - edgesize) return [255 - i, 0, i, 255];
	return [127, 0, 127, 255];
});
procedural_canvas('#invert', function(i, j, h, w) {
    if (i < h / 2) {
        return [j, j, j, 255];
    }

    var c = to8bit(linear2gamma(1 - gamma2linear(j / (w - 1))));
    return [c, c, c, 255];
});

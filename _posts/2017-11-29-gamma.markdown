...
(Unfortunatley, if you are reading this on a high-DPI device like a smart phone
or a newish macbook, your browser is upscaling that image and ruining the effect
I'm going for. All of the checkerboard examples in this article will look wrong.
Sorry.)
...
function procedural_canvas(selector, callback) {
    var pixelRatio = 1;
    if (window.devicePixelRation) pixelRatio = window.devicePixelRatio;
	...
	for (var k = 0; k < canvases.length; ++k) {
		...
		for (var y = 0; y < height; y += pixelRatio) {
			for (var x = 0; x < width; x += pixelRatio) {
				var color = callback(y, x, height, width);
                for (var ty = y; ty < y + pixelRatio; ++ty) {
                    for (var tx = x; tx < x + pixelRatio; ++tx) {
                        for (var c = 0; c < 4; ++c) image.data[(ty * width + tx) * 4 + c] = color[c];
                    }
                }
			}
		}

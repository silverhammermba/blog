...
<canvas class="checkers" width="200" height="200"></canvas>
<canvas id="scaled" width="200" height="200" style="width: 100px; height: 100px"></canvas>
...WHAT!?
...
If you're reading this article on a high-DPI screen such as a smart phone or a
...
it ends up _tiny_ because the pixels are smaller. To compensate, these devices
...
checkerboard patterns (called [dither][d]) so you end up with a distorted
...
issue, it's just a [moir&eacute; patten][m] likely due to how the display
scaling interacts with the HTML canvas scaling and the checkerboard.

[d]: https://en.wikipedia.org/wiki/Dither
[m]: https://en.wikipedia.org/wiki/Moir%C3%A9_pattern
...
procedural_canvas('#scaled', function(y, x) {
	/* XXX this is supposed to look bad, so don't bother using the workaround
	 * for black on high-DPI
	 */

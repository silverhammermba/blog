...
brights meaning we can store the maximum amount of _discernable differences in
...
it's more like 21% brightness in terms of light intensity. This works great as
long as you're working with only a single color channel at a time (or all of
them at once) because then the change in appearance as you tweak the numerical
values matches exactly with your (again, naive) expectation. Want to make a shade
of gray look half as bright? Halve its RGB values! Want to make a shade of
orange look three times as bright? Triple its green channel! In some limited
situations, the wrong behavior is actually intuitive. But if you're
working with different colors where the two have little to no difference in
brightness (like the previous red/blue example), sRGB no longer matches our
intuition and we're forced to face reality: perceived brightness &ne;
light intensity.
...
users' expectations.
    ...
    convert -size somesize gradient:somegradient -set colorspace RGB -colorspace sRGB out

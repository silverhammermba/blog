...
single color should be [127, 127, 127]. Right?
...
it to demonstrate its understanding of light intensity.
...
**correct** in some cases; mixing colors in the sRGB space is **always incorrect**. This
is about sRGB being more **intuitive** in situations where the naive
assumption that perceived brightness and light intensity are the same holds.
...
correction (they call this `rec709luma`). Unfortunately, that is the formula used
...
the smaller case (making small fonts less readable).
...
stuff. Usually people described sRGB as "nonlinear RGB" color space. And when
...
linear one and the "linear RGB" is now nonlinear! See, it's confusing. The point
is that humans like to think in terms of linear spaces. sRGB is linear in the
perceptual space and "linear RGB" is linear in the light intensity space and
both of them are nonlinear when viewed from the perspective of the other space.
...
procedural_canvas('#compare', function(y, x, h, w) {
	...
	if (x > edgesize && x <= w / 2) {
		var color = to8bit(linear2gamma(y / 255));
		return [color, color, color, 255];
	}
	if (x > w / 2 && x < w - edgesize) return [y, y, y, 255];
	return [127, 127, 127, 255];
});
...
procedural_canvas('#compare-rgbry', function(y, x, h, w) {
	...
	if (x > w / 2 && x < w - edgesize) return [255, y, 0, 255];
...
procedural_canvas('#compare-rgbrb', function(y, x, h, w) {
	...
	if (x > edgesize && x <= w / 2) {
		var c1 = to8bit(linear2gamma(y / 255));
		var c2 = to8bit(linear2gamma(1 - y / 255));
		return [c2, 0, c1, 255];
	}
	if (x > w / 2 && x < w - edgesize) return [255 - y, 0, y, 255];
